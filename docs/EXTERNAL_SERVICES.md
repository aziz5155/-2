...
## 1.2 نشر نسخة الويب كموقع عام حقيقي (Vercel — مجاني)

المشروع أساسًا تطبيق جوال، لكنه يُصدَّر أيضًا كموقع ويب كامل (نفس الشاشات
والتصميم، عبر React Native Web) — هذا يعطيك رابطًا دائمًا وثابتًا تنشره
للجميع، يتحدّث تلقائيًا مع كل تعديل على الكود:

1. أنشئ حسابًا على [vercel.com](https://vercel.com) بالضغط على **Continue
   with GitHub** (تستخدم نفس حساب GitHub اللي فيه مستودع المشروع، بدون
   بطاقة ائتمان).
2. اضغط **Add New... → Project**.
3. اختر مستودعك (`aziz5155/-2`) من القائمة واضغط **Import**.
4. Vercel يكتشف إعدادات البناء تلقائيًا من ملف `vercel.json` الموجود في
   المشروع — لا تغيّر شي في خانات Build Command / Output Directory.
5. **قبل ما تضغط Deploy**، افتح قسم **Environment Variables** وأضف متغيّرين
   (نفس القيم اللي أعطيتني إياها لملف `.env`):
   - `EXPO_PUBLIC_SUPABASE_URL`
   - `EXPO_PUBLIC_SUPABASE_ANON_KEY`
6. اضغط **Deploy** وانتظر دقيقة أو دقيقتين.
7. يعطيك رابطًا جاهزًا يشبه `https://your-project.vercel.app` — هذا رابطك
   العام النهائي، شاركه مع من تحب.
8. (اختياري لاحقًا) من إعدادات المشروع في Vercel → **Domains**، تقدر تربط
   نطاقًا خاصًا اشتريته (مثل `familysuccess.app`) بدل الرابط الافتراضي.

⚠️ نسخة الويب لا تدعم "تسجيل الدخول بـ Apple" (خاص بالجوال فقط) — باقي كل
الوظائف (البريد، لوحة المالك، المهام، المكافآت...) تعمل بشكل طبيعي.
